...
function List(props) {
  ...
  const pages = Math.ceil((props.list?.length || 0) / pageItems);

  const displayedItems = completed
    ? (props.list || []).filter((item) => !item.complete)
    : props.list || [];

  const sortedItems = sort === 'difficulty'
    ? [...displayedItems].sort((a, b) => a.difficulty - b.difficulty)
    : displayedItems;

  const firstItem = (currentPage - 1) * pageItems;
  const lastItem = currentPage * pageItems;
  const finalItems = sortedItems.slice(firstItem, lastItem);
